...
public class SubroutineHierarchy extends JPanel {
	...
	private void drawWorld(Graphics2D g2) {
		...
		ngon(g2, 100, -1.02, -0.05);
		ngon(g2, 100, 1.04, -0.98);
		ngon(g2, 80, -1.379, 1.40);
		ngon(g2, 80, -3.13, 2.23);
		ngon(g2, 60, 0.9, 2.05);
		ngon(g2, 60, 2.12, 1.45);
		
		T(g2 ,0.5,0.5,0,-2,Color.BLUE);
		T(g2 ,0.35,0.35,-2.25,0.75,new Color(128, 0, 128));
		T(g2 ,0.25,0.25, 1.5,1, Color.GREEN);
	...
	private void S_Line (Graphics2D g2,double x , double y, double translate_x ,double translate_y, Color color  )
	{
		AffineTransform saveTransform = g2.getTransform();
		g2.scale(x,y);
		L(g2, translate_x, translate_y);
		g2.setTransform(saveTransform);
	}
	private void L (Graphics2D g2, double translate_x ,double translate_y  )
	{
	...
	private void T (Graphics2D g2,double x , double y, double translate_x ,double translate_y ,Color color )
	{
		AffineTransform saveTransform = g2.getTransform();
		g2.setColor(color);
		g2.translate(translate_x,translate_y);
	...
	private void ngon(Graphics2D g2,double r, double translate_x ,double translate_y) // polygon
	{
		AffineTransform saveTransform = g2.getTransform();
		...
		for(int i=0;i<n;i++)
		{
			g2.drawLine( x1[i],y1[i],0,0 );//srednice
		}
		g2.draw(polygon);
