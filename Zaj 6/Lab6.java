...
public class Lab6 extends GLJPanel implements GLEventListener  {
	...
	public void display(GLAutoDrawable drawable) {    
		...
		int verticles = 15;
        double increment = 2 * Math.PI/verticles;
        double angle =0;
        int radius =6;
        float x =0;
        float z = 0;

        gl2.glMaterialfv( GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT, materials[14], 0);
        gl2.glMaterialfv( GL2.GL_FRONT_AND_BACK, GL2.GL_DIFFUSE, materials[14], 4 );
        gl2.glMaterialfv( GL2.GL_FRONT_AND_BACK, GL2.GL_SPECULAR, materials[14], 8 );
        gl2.glMaterialf( GL2.GL_FRONT_AND_BACK, GL2.GL_SHININESS, materials[14S][12] );
        ...
        for (int i = 1; i <= verticles; i++) {
            ...
            angle = i * increment;
            float x0 = (float) (radius * Math.cos(angle));
            float z0 =(float) (radius * Math.sin(angle));
            float[] U = {x-0,-2.0f-8,z-0};
            float[] V = {x-x0,0,z-z0};
            gl2.glNormal3f(U[1]*V[2]-U[2]*V[1],U[2]*V[0]-U[0]*V[2],U[0]*V[1]-U[1]*V[0]);
            gl2.glVertex3f( x, -2.0f, z );
            gl2.glVertex3f( 0, 8, 0 );
            gl2.glVertex3f(x0, -2.0f,z0);
	...
	public void init(GLAutoDrawable drawable) {
		...
		float[] blue1  = {0.2f, 0.2f, 0.2f, 1};
        float[] blue2  = {0.4f, 0.4f, 0.4f, 1};
        float[] blue3  = {0.15f, 0.15f, 0.15f, 1};
        gl2.glLightfv (GLLightingFunc.GL_LIGHT1, GLLightingFunc.GL_DIFFUSE, blue1,0);
        gl2.glLightfv (GLLightingFunc.GL_LIGHT1, GLLightingFunc.GL_SPECULAR, blue2,0);
        gl2.glLightfv (GLLightingFunc.GL_LIGHT1, GLLightingFunc.GL_AMBIENT, blue3,0);
        float[] position = {1,0,0,0};
        gl2.glLightfv (GLLightingFunc.GL_LIGHT1, GLLightingFunc.GL_POSITION, position, 0);
